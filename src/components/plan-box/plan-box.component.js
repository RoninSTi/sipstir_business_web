...
const PlanActive = () => {
  ...
  const isLoading = useSelector(state => state.ui.isLoading.some(element => element.loadingType === CANCEL_SUBSCRIPTION))
  ...
  return (
    <div className='mb-5'>
      ...
      <br />
      <span className='has-text-weight-semibold'>{`Auto renews ${moment(subscription?.currentPeriodEnd * 1000).format('MMMM DD, YYYY')}`}</span>
      <br />
      <button
...
const PlanBox = props => {
  ...
  return (
    <div className='container'>
      ...
      {paymentMethod ? (
        <div>
          <span className='has-text-weight-semibold is-size-5'>{`${paymentMethod?.brand.charAt(0).toUpperCase()}${paymentMethod?.brand.slice(1)}`}</span>
          <br />
