...
const BusinessRow = ({ business }) => {
 ...
 const isDeleting = useSelector((state) =>
  state.ui.isLoading.some(
   (element) => element.loadingType === DELETE_BUSINESS && element.meta === id,
  ),
 );
 ...
 const handleDeleteBusiness = () => {
  dispatch({
   ...
   payload: {
    ...
    message:
     'Are you sure you want to delete this business?  They will no longer be able to participate in our program.',
  ...
  <tr>
   ...
   <td>
    <div className="buttons">
     <Link className="button is-small" to={`accounts/${id}`}>
      Edit
     </Link>
     <button
